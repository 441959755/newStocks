...
export default class NewClass extends cc.Component {
    ...
    isZhangTing(price) {
        ...
        if ((this.curData.close * p) < price) {
            GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '委托价格不能超过涨停价格！');
            return false;
        }
        return true;
    }
    ...
    onBtnClick(event) {
        ...
        //当前交易价格  减
            ...
            if (!this.kmsl) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '您的资产不足');
                return;
            }
        ...
        //买入下单
            ...
            if (!this.cursl) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '买入数量不能为空');
                return;
            }
            ...
            (<any>window).socket.send(pb.MessageId.Req_Game_Order, buff, (res) => {
                ...
                if (res.orderId) {
                    GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '买入下单成功!');
