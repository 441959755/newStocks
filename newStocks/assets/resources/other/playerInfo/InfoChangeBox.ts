...
@ccclass
export default class InfoChangeBox extends cc.Component {
    ...
    selectLocation(node, da?) {
        ...
        if (da == 1) {
            ...
            this.onSendMessage(id, data, call);
    ...
    onSendMessage(id, data, call?) {
        ...
        (<any>window).socket.send(id, buff, (info) => {
            ...
            if (info.err) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, info.err);
            } else {
                call && (call())
    ...
    testTestlength(v) {
        if (v == GameData.userName) {
            GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '输入的昵称不要跟以前的一样');
            return false;
        }
        ...
        if (flag != 3) {
            if (num > 12) {
                ...
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '昵称长度最多为6个汉字或12个字母数字！');
                return false;
            } else if (num < 1) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '输入的昵称不能为空！');
                return false;
            }
            return true;
        } else {

            GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '输入的昵称不能包含特殊符号！');
            return false;
        }
