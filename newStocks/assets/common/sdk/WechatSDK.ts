...
export default class WechatSDK {
    ...
    getUserInfo1(code, btn, call) {
        ...
        wx.getUserInfo({
            success(res) {
                ...
                //     GameData.headImg = sp;
                // }, true)
                console.log('图片地址：' + GameData.headImgurl);
                LoadUtils.loadRemote(GameData.headImgurl).then((res) => {
                    GameData.headImg = new cc.SpriteFrame(res);
                })
    ...
    onLoginCodeHttpRequest(code, call) {
        ...
        HttpMgr.loginHttp(code, loginInfo, call)
    ...
    showVideoAd(callback?) {
        console.log(GameData.appConf.wechat_minprogram + '   ' + JSON.stringify(GameData.appConf.wechat_minprogram));
        this.callback = callback;
        if (!GameData.appConf.wechat_minprogram.ad_switch) {
            this.callback(1);
        }
        else {
            if (this.videoAd) {
    ...
    onShareAppMessage() {
        ...
        wx.onShareTimeline(() => {
            return {
                ...
                imageUrl: LLWConfing.LoginUrl + '/wechatgame/share.png',
                query: 'a=1&b=2'
            }
        })
    ...
    shareAppMessage(type) {
        ...
        if (type == 1) {
            url = LLWConfing.LoginUrl + '/wechatgame/share.png';
            title = '【炒股达人】寓教于乐的炒股软件';
            uid = GameData.userID;
        }
        else {
            url = LLWConfing.LoginUrl + '/wechatgame/invite.png';
            title = '比拼K线技术，等你来挑战';
            uid = GameData.userID;
