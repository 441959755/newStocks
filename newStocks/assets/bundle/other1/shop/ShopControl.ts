...
export default class ShopControl extends cc.Component {
    ...
    onLoad() {
        ...
        this.editBox.node.on('editing-did-ended', edit => {
            ...
            if (this.isPhoneNumber(str)) {
                ...
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '输入的手机号有误，请重新输入');
                this.editBox.string = '';
                return;
            }
        }, this);
    ...
    onBtnClick(event, curdata) {
        ...
        if (name == 'black') {
            ...
            GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '微信号已复制');
        ...
        //奖券
            ...
            if (this.getBindMobild()) {
                if (GameData.properties[pb.GamePropertyId.Coupon] > Math.abs(this.couponCfg[index].costs[0].v)) {
                    ...
                    GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '点券不足');
                }
            }
            else {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '请先绑定手机号码');
            }
        }
    }
