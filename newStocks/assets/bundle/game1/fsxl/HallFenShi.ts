...
export default class HallFenShi extends cc.Component {
    ...
    checkCond() {
        ...
        let times = ConfUtils.getTimeByCodeName1(datas[0]);
        if (!times) { return }
        if (times.end < TimeUtils.fromatTime1((new Date().getTime() - 14 * 24 * 60 * 60 * 1000) / 1000)) {
            GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '没有最新能两周数据');
            this.editbox.string = '';
            this.editbox.placeholder = '随机选股';
        }
    }
    ...
    onStartGame() {
        ...
        if (this.boxsLa[0].string == '随机' || this.boxsLa[1].string == '--' || this.boxsLa[2].string == '--') {
            ...
            let ts = TimeUtils.getTimestamp(tDate);
        ...
        if (this.editbox.string == '随机选股' || this.editbox.string == '') {
            items1 = ConfUtils.getItemsByTime1();
            code = items1[0];
        }
        else {
            // items1 = GameCfgText.getItemsByTime1();
            // code = items1[0];
            items1 = ConfUtils.getGPItemInfo(GameData.fsSet.search);
            code = items1[0];
        }
       ...
       // let message = pb.CmdQuoteQuery.create(info1);
        ...
        (<any>window).socket.send(pb.MessageId.Req_QuoteQuery, buff, info => {
            ...
            if (info.items.length <= 0) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '行情为空');
                return;
            }
            ...
            GameBundle.loadPre('fsxl/GameFenShi');
        })
