...
@ccclass
export default class RoomHandle extends cc.Component {
    ...
    onShow(info?) {
...

        {
            ...
            exp.string = '经验值：' + GameData.properties[pb.GamePropertyId.Exp] + '/' +
                GameData.gameConf.level_exp[(GameData.properties[pb.GamePropertyId.Level] || 1)];
...

        {
            ...
            if (GameData.Players && GameData.Players[1] && GameData.Players[1].nickname) {
                ...
                exp.string = '经验值：' + GameData.Players[1].properties[pb.GamePropertyId.Exp] + '/' +
                    GameData.gameConf.level_exp[(GameData.Players[1].properties[pb.GamePropertyId.Level] || 1)];
                read.string = '等待准备...';
    ...
    onLoadHead(ob, head) {
        ...
        LoadUtils.loadHeadImg(GameData.otherHead, head.getComponent(cc.Sprite));
    }
        ...
        //点击在线邀请
            ...
            this.cb = setInterval(() => {
                if (count <= 0) {
                    ...
                    count--;
                    timeLabel.string = TimeUtils.onNumChangeTime(count);
                }
            }, 1000);
        }
            ...
            if (!this.zbFlag) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '玩家还没准备');
                return;
            }
            ...
            (<any>window).socket.send(pb.MessageId.Req_Room_Ready, buff, (res) => {
