...
@ccclass
export default class ZnDrawGraphics extends cc.Component {
    ...
    setTiemNode() {
        if (this.ktype == pb.KType.Min) {
            ...
            this.timelas.forEach((el, index) => {
                el.string = TimeUtils.formatTime(arr[index]);
            })
        }
    }
    ...
    onDrawMA(index) {
        ...
        if (!this.MaList[index]) {
            //    console.log('this.MaList is null:' + this.MaList + '   ' + index);
            return;
        }
    ...
    drawLine(ctx, sX, sY, eX, eY, falg?) {
        DrawUtils.drawLine(ctx, sX, sY, eX, eY);
    }
