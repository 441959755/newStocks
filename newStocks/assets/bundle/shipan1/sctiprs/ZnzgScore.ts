...
import PERatiosCfg from "../PERatiosCfg";
...
export default class ZnzgScore extends cc.Component {
    ...
    getScore() {
        ...
        let hypjsyl = PERatiosCfg.info[ZnzgControl.csrc_indu_name][0];

        if (ZnzgControl.syl > hypjsyl * 1.5) {
            score1 += (1 * 0.1);
        }

        else if (ZnzgControl.syl > hypjsyl && ZnzgControl.syl < hypjsyl * 1.5) {
            score1 += (3 * 0.1);
        }
        ...
        if (ZnzgControl.ni_ded_yoy_gr < 0) {
            score1 += (1 * 0.3);
        }
        else if (ZnzgControl.ni_ded_yoy_gr < 0.2) {
            score1 += (2 * 0.3);
        }
        else if (ZnzgControl.ni_ded_yoy_gr < 0.4) {
            score1 += (3 * 0.3);
        }
        else if (ZnzgControl.ni_ded_yoy_gr < 0.6) {
            score1 += (4 * 0.3);
        } else {
            score1 += (5 * 0.3);
        }
        ...
        if (ZnzgControl.lastStockIndexList[ZnzgControl.lastStockIndexList.length - 1].index - gpData.length >= 10) {
            score3 += (4 * 0.15);
        }
        else {
            score3 += (1 * 0.15);
        ...
        this.starSp[0].fillRange = score1 / 5 //* 0.2;
        this.starSp[1].fillRange = score2 / 5 //* 0.2;
        this.starSp[2].fillRange = score3 / 5 //* 0.4;
     ...
     */
    getStockQuotes() {
        if (!ZnzgControl.searchCode) { return }
        GlobalHandle.getHttpGPData('k', ZnzgControl.searchCode, 1000, (type, res) => {
            // amount: 38520
            // close: 6.86
            // high: 6.96
            // low: 6.75
            ...
            res.forEach(el => {
                ...
                StockData.gpDataDay.push(data);
            });

            GameCfg.data[0].data = StockData.gpDataDay;
