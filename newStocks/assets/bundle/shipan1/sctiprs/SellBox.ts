...
@ccclass
export default class SellBox extends cc.Component {
    ...
    onBtnClick(event, data) {
        ...
        //交易价格减
            ...
            if (!this.curSl) {
                ...
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '没有持仓数量!');
                return;
            }
        ...
        //加
            ...
            if (this.curSellCount + 100 > this.curSl) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '不能超出持仓数量!');
                return;
            }
        ...
        //卖出下单
            ...
            if (!this.curSellCount) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '卖出下单数量不能为0');
                return;
            }
            ...
            (<any>window).socket.send(pb.MessageId.Req_Game_Order, buff, (res) => {
                ...
                if (res.orderId) {
                    ...
                    GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '卖出下单成功!');
    ...
    isDieTing(price) {
        ...
        if ((this.curData.close * p) > price) {
            GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '委托价格不能低于跌停价格！！');
            return false;
        }
        return true;
    }
