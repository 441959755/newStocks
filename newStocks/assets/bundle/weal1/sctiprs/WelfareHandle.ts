            ...
            edit => {
                ...
                if (str == '') {
                    ...
                    if (parseInt(str) > GameData.properties[pb.GamePropertyId.K]) {
                        GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '兑换K币不能超可用k币数量！')
    ...
    onBtnClick(event, curData) {
        ...
        if (name == 'closeBtn') {
            ...
            if (this.editbox.string == '') {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '请输入用户ID!');
                return;
            }

            if (this.editbox1.string == '') {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '请输入要兑换的K币!');
                return;
            }

            if (parseInt(this.editbox1.string) < 10) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '兑换的K币要大于等于10');
                return;
            }
