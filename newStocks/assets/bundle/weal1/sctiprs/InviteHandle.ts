...
export default class InviteHandle extends cc.Component {
    ...
    onBtnClick(event, customData) {
        ...
        //领取金币
            ...
            if (!this.inviterData) {
                GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '暂无数据');
            }
            else {
        ...
        (<any>window).socket.send(pb.MessageId.Req_Hall_GetInviterAward, buff, (res) => {
            ...
            if (res.err) {
                ...
                if (type == pb.GamePropertyId.Gold) {
                    GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '成功领取:' + count * 500 + '金币！');
                }
                else {
                    GlobalEvent.emit(EventCfg.SHOWTIPSTEXT, '成功领取:' + (count / 10) * 3 + '天VIP');
                }
            }
        })
    }
